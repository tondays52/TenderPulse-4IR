...
// Footer with smart page detection
// Single-page: no footer
// Multi-page page 1: page number only
// Multi-page page 2+: centered author · cvdate | right-aligned page/total
#let footer-content(author, cvdate, light-color, footer-size: 9pt, body-font-str: "Source Sans 3") = context [
  #if counter(page).final().first() > 1 [
    // Multi-page letter
    #if counter(page).get().first() == 1 [
      // Page 1 of multi-page: page number only
      ...
      // Page 2+ of multi-page: full footer with author and date
      #text(font: body-font-str, size: footer-size, fill: light-color)[
        #box(width: 100%)[
          #align(center)[
            #{
...
#let moderncv(
     ...
     #grid(columns: (1fr), row-gutter: 0pt)[
        #text(
          ...
          #display_name
           ...
           #v(-6pt)
           #text(font: heading-font-str, size: 9pt, weight: 700, fill: accent)[#research-area]
         ]
            #if location != none [
              #v(-8pt)
               #text(font: heading-font-str, size: contact-size, weight: 700, fill: main_color)[#location]
             ]
  ...
  // Recipient/Date table (letter-specific, full page width, page 1 only, outside main block)
  #context if counter(page).get().first() == 1 and (not is-empty(letterdate) or not is-empty(letter_recipient)) [
    #v(10pt)
    #table(
      columns: (1fr, auto),
      inset: 0pt,
      ...
      column-gutter: 2em,
      [
        #set text(size: body-size, fill: main_color)
        #letter_recipient
      ],
      [
        #set text(size: body-size, fill: main_color)
        #if not is-empty(letterdate) [
          #letterdate
        ]
      ]
    )
